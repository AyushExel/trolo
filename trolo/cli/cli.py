...
def train(config, model, dataset, pretrained, device, batch_size):
    ...
    # Initialize trainer
    trainer = DetectionTrainer(config=config, model=model, dataset=dataset, pretrained_model=pretrained, **overrides)
...
def predict(model, input, output, device, conf_thresh, save, no_show):
    ...
    predictor = DetectionPredictor(model=infer_pretrained_model(model), device=device or infer_device())
