...
class DetectionTrainer(BaseTrainer):
    ...
    def __init__(
        ...
        **overrides,
        ...
        """
        super().__init__(
            config=config, model=model, dataset=dataset, pretrained_model=pretrained_model, loggers=loggers, **overrides
        )
    ...
    def val(self, device: str = None):
        ...
        module = self.ema.module if self.ema else self.model
        test_stats, coco_evaluator = evaluate(
            module, self.criterion, self.postprocessor, self.val_dataloader, self.evaluator, device or self.device
        )
